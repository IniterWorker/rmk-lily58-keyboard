...
use rmk::{
    initialize_keymap_and_storage, run_devices, run_processor_chain, run_rmk, HostResources,
};
...
async fn main(spawner: Spawner) {
    ...
    // Note: because of the heat of the leds (sk, ws2812...)
    // You will need to use the external xtal to reduce led timing issue
    ...
    join4(
        ...
        keyboard.run(),
        join(
            run_peripheral_manager::<5, 6, 5, 0, _>(0, peripheral_addrs[0], &stack),
            run_rmk(
                &keymap,
                driver,
                &stack,
